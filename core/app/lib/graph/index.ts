...
import { Node, Edge } from "@/src/components/workflow/types";
 ...
 */
export function createAdjacencyList(nodes: Node[], edges: Edge[]): Map<string, string[]> {
 ...
 */
export function detectCycle(nodes: Node[], edges: Edge[]): { hasCycle: boolean; cycle?: string[] } {
  const WHITE = 0;
  const GRAY = 1;
  const BLACK = 2;
  ...
  if (cycleDetected && cycleStart && cycleEnd) {
    ...
    cycle.unshift(cycleStart); // Complete the cycle
...
export function topologicalSort(nodes: Node[], edges: Edge[]): string[] | null {
  ...
  if (hasCycle) {
    return null; // Can't do topological sort on a cyclic graph
  }
...
export function getExecutionOrder(nodes: Node[], edges: Edge[]): {
  ...
  const { hasCycle, cycle } = detectCycle(nodes, edges);
...
export function findAllPaths(
  ...
  function dfs(currentId: string, visited = new Set<string>(), path: string[] = []) {
 ...
 */
export function getWorkflowExecutionPlan(nodes: Node[], edges: Edge[]): {
  plan: string[] | null;
  ...
  branchPoints.forEach(startId => {
    mergePoints.forEach(endId => {
      ...
      if (paths.length > 1) {
        ...
        const nonEmptyBranches = parallelBranches.filter(branch => branch.length > 0);
